...
///
/// Worker sends peer batch to EL for validation.
#[derive(Debug, Clone)]
pub struct BatchTransactionValidator<Client, T> {
    /// The type that performs the actual validation.
    inner: Arc<BatchTransactionValidatorInner<Client, T>>,
...
impl<Client, Tx> BatchTransactionValidator<Client, Tx> {
    ...
    where
        T: TaskSpawner,
    {
        let inner = BatchTransactionValidatorInner {
            chain_spec,
            client,
            shanghai: true,
            eip2718: true,
...
impl<Client, Tx> TransactionValidator for BatchTransactionValidator<Client, Tx>
where
...
#[derive(Debug, Clone)]
struct BatchTransactionValidatorInner<Client, T> {
    /// Spec of the chain
    chain_spec: Arc<ChainSpec>,
...
impl<Client, Tx> BatchTransactionValidatorInner<Client, Tx> {
    /// Returns the configured chain id
...
#[async_trait::async_trait]
impl<Client, Tx> TransactionValidator for BatchTransactionValidatorInner<Client, Tx>
where
    ...
    Tx: PoolTransaction,
{
    ...
    async fn validate_transaction(
        &self,
        _origin: TransactionOrigin, // only used for external batch txs
        transaction: Self::Transaction,
    ) -> TransactionValidationOutcome<Self::Transaction> {
        // Checks for tx_type
