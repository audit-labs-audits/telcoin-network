...
async fn test_reopen(#[values(true, false)] is_transactional: bool) {
    ...
    let db = RocksDBMap::<u32, String>::reopen(&arc.rocksdb, None, &ReadWriteOptions::default())
...
async fn test_reopen_macro() {
    ...
    let rocks = open_cf(temp_dir(), None, MetricConf::default(), &[FIRST_CF, SECOND_CF]).unwrap();
...
async fn test_wrong_reopen(#[values(true, false)] is_transactional: bool) {
    ...
    let db = RocksDBMap::<u8, u8>::reopen(&rocks, Some("quux"), &ReadWriteOptions::default());
    assert!(db.is_err());
}
...
async fn test_try_extend(#[values(true, false)] is_transactional: bool) {
    ...
    db.try_extend(&mut keys_vals).expect("Failed to extend the DB with (k, v) pairs");
...
async fn test_insert_batch_across_different_db(#[values(true, false)] is_transactional: bool) {
    ...
    let db_cf_1: RocksDBMap<i32, String> =
        RocksDBMap::reopen(&rocks, Some("First_CF"), &ReadWriteOptions::default())
            .expect("Failed to open storage");
    let keys_vals_1 = (1..100).map(|i| (i, i.to_string()));

    let db_cf_2: RocksDBMap<i32, String> =
        RocksDBMap::reopen(&rocks2, Some("Second_CF"), &ReadWriteOptions::default())
            .expect("Failed to open storage");
    let keys_vals_2 = (1000..1100).map(|i| (i, i.to_string()));
...
async fn test_delete_batch() {
    let db = RocksDBMap::<i32, String>::open(
        temp_dir(),
        MetricConf::default(),
        None,
        None,
...
async fn test_delete_range() {
    let db: RocksDBMap<i32, String> = RocksDBMap::open(
        temp_dir(),
        MetricConf::default(),
        None,
        None,
...
async fn test_multi_insert(#[values(true, false)] is_transactional: bool) {
    // Init a DB
    let db: RocksDBMap<i32, String> = open_map(temp_dir(), Some("table"), is_transactional);
    // Create kv pairs
    let keys_vals = (0..101).map(|i| (i, i.to_string()));
...
async fn test_checkpoint(#[values(true, false)] is_transactional: bool) {
    ...
    let db: RocksDBMap<i32, String> = open_map(db_path, Some("table"), is_transactional);
    // Create kv pairs
    let keys_vals = (0..101).map(|i| (i, i.to_string()));
    ...
    // Verify checkpoint
    let checkpointed_db: RocksDBMap<i32, String> =
    ...
    // Ensure keys inserted after checkpoint are only present in original db but not in checkpointed
    // db
...
async fn test_transactional() {
    ...
    let db = RocksDBMap::<String, String>::reopen(&rocksdb, None, &ReadWriteOptions::default())
        .expect("Failed to re-open storage");
    ...
    tx1.insert_batch(&db, vec![(key.to_string(), "1".to_string())]).unwrap();
...
async fn test_transaction_snapshot() {
    ...
    // write.
    let mut tx1 = db.transaction_without_snapshot().expect("failed to initiate transaction");
    ...
    tx1.insert_batch(&db, vec![(key.to_string(), "2".to_string())]).unwrap();
    db.insert(&key, &"1".to_string()).unwrap();
    assert!(matches!(tx1.commit(), Err(TypedStoreError::RetryableTransactionError)));
    ...
    // writes.
    let mut tx1 = db.transaction_without_snapshot().expect("failed to initiate transaction");
    let mut tx2 = db.transaction_without_snapshot().expect("failed to initiate transaction");
    tx1.insert_batch(&db, vec![(key.to_string(), "1".to_string())]).unwrap();
    tx2.insert_batch(&db, vec![(key.to_string(), "2".to_string())]).unwrap();
    // which ever tx is committed first will succeed.
    tx1.commit().expect("failed to commit");
    assert!(matches!(tx2.commit(), Err(TypedStoreError::RetryableTransactionError)));
...
async fn test_retry_transaction() {
    ...
    tokio::time::timeout(Duration::from_secs(1), async move {
        retry_transaction_forever!({
            let mut tx1 =
                db.transaction_without_snapshot().expect("failed to initiate transaction");
            tx1.insert_batch(&db, vec![(key.to_string(), "2".to_string())]).unwrap();
            db.insert(&key, &"1".to_string()).unwrap();
            tx1.commit()
        })
...
async fn test_transaction_read_your_write() {
    ...
    tx.insert_batch(
        &db,
        vec![(key1.to_string(), "11".to_string()), (key2.to_string(), "2".to_string())],
    )
    .unwrap();
    assert_eq!(db.get(&key1.to_string()).unwrap(), Some("1".to_string()));
    assert_eq!(db.get(&key2.to_string()).unwrap(), None);

    assert_eq!(tx.get(&db, &key1.to_string()).unwrap(), Some("11".to_string()));
    assert_eq!(tx.get(&db, &key2.to_string()).unwrap(), Some("2".to_string()));

    tx.delete_batch(&db, vec![(key2.to_string())]).unwrap();

    assert_eq!(
        tx.multi_get(&db, vec![key1.to_string(), key2.to_string()]).unwrap(),
...
async fn open_as_secondary_test() {
    ...
    // Init a DB
    let primary_db = RocksDBMap::<i32, String>::open(
        primary_path.clone(),
        MetricConf::default(),
        None,
        Some("table"),
    ...
    let opt = rocksdb::Options::default();
    let secondary_store =
        open_cf_opts_secondary(primary_path, None, None, MetricConf::default(), &[("table", opt)])
            .unwrap();
    let secondary_db = RocksDBMap::<i32, String>::reopen(
        &secondary_store,
        Some("table"),
        &ReadWriteOptions::default(),
    )
    ...
    V: Serialize + DeserializeOwned,
{
    if is_transactional {
        ...
        .map(|db| RocksDBMap::new(db, &ReadWriteOptions::default(), cf))
        .expect("failed to open rocksdb")
    } else {
        RocksDBMap::<K, V>::open(
            path,
...
fn open_rocksdb<P: AsRef<Path>>(path: P, opt_cfs: &[&str], is_transactional: bool) -> Arc<RocksDB> {
    if is_transactional {
        ...
        let cfs: Vec<_> = opt_cfs.iter().map(|name| (*name, options.clone())).collect();
