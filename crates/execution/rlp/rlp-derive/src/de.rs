...
pub(crate) fn impl_decodable(ast: &syn::DeriveInput) -> Result<TokenStream> {
    ...
    let impl_block = quote! {
        impl #impl_generics execution_rlp::Decodable for #name #ty_generics #where_clause {
            fn decode(mut buf: &mut &[u8]) -> Result<Self, execution_rlp::DecodeError> {
                let b = &mut &**buf;
    ...
    let impl_block = quote! {
        impl #impl_generics execution_rlp::Decodable for #name #ty_generics #where_clause {
            fn decode(buf: &mut &[u8]) -> Result<Self, execution_rlp::DecodeError> {
                Ok(Self(execution_rlp::Decodable::decode(buf)?))
            }
        }
    };
...
fn decodable_field(index: usize, field: &syn::Field, is_opt: bool) -> TokenStream {
    ...
    if attributes_include(&field.attrs, "default") {
        ...
        quote! {
            #ident: if started_len - b.len() < rlp_head.payload_length {
                if b.first().map(|b| *b == #EMPTY_STRING_CODE).unwrap_or_default() {
                    ...
                    Some(execution_rlp::Decodable::decode(b)?)
                }
            } else {
                None
            },
        ...
        quote! { #ident: execution_rlp::Decodable::decode(b)?, }
    }
}
