...
async fn commit_one_with_leader_schedule_change() {
    ...
    for mut test_case in test_cases {
        ...
        let genesis =
            Certificate::genesis(&committee).iter().map(|x| x.digest()).collect::<BTreeSet<_>>();
        let (certificates, _next_parents) = narwhal_types::test_utils::make_optimal_certificates(
            &committee,
            1..=test_case.rounds,
            &genesis,
            &ids,
...
async fn not_enough_support_with_leader_schedule_change() {
    ...
    leader_configs.insert(
        6,
        narwhal_types::test_utils::TestLeaderConfiguration {
            round: 6,
            authority: AuthorityIdentifier(2),
            should_omit: false,
            support: Some(narwhal_types::test_utils::TestLeaderSupport::Weak),
        },
    );
    ...
    leader_configs.insert(
        ...
        narwhal_types::test_utils::TestLeaderConfiguration {
            ...
            should_omit: false,
            support: Some(narwhal_types::test_utils::TestLeaderSupport::NoSupport),
        },
    );
    ...
    let (out, _parents) = narwhal_types::test_utils::make_certificates_with_leader_configuration(
        &committee,
        1..=15,
        &genesis,
        &ids,
...
async fn dead_node() {
    ...
    let (mut certificates, _) =
        narwhal_types::test_utils::make_optimal_certificates(&committee, 1..=11, &genesis, &ids);
...
async fn not_enough_support() {
    ...
    let (out, parents) =
        narwhal_types::test_utils::make_optimal_certificates(&committee, 1..=1, &genesis, &nodes);
    certificates.extend(out);
    ...
    let (leader_2_digest, certificate) =
        narwhal_types::test_utils::mock_certificate(&committee, ids[0], 2, parents.clone());
    certificates.push_back(certificate);

    let nodes: Vec<_> = ids.iter().skip(1).cloned().collect();
    let (out, mut parents) =
        narwhal_types::test_utils::make_optimal_certificates(&committee, 2..=2, &parents, &nodes);
    certificates.extend(out);
    ...
    let (digest, certificate) =
        narwhal_types::test_utils::mock_certificate(&committee, name, 3, parents.clone());
    certificates.push_back(certificate);
    ...
    let (out, parents) =
        narwhal_types::test_utils::make_optimal_certificates(&committee, 4..=4, &parents, &nodes);
    certificates.extend(out);

    // Round 5: Send f+1 certificates to trigger the commit of leader 4.
    let (_, certificate) =
        narwhal_types::test_utils::mock_certificate(&committee, ids[0], 5, parents.clone());
    certificates.push_back(certificate);
    let (_, certificate) =
        narwhal_types::test_utils::mock_certificate(&committee, ids[1], 5, parents);
    certificates.push_back(certificate);
...
async fn missing_leader() {
    ...
    let nodes: Vec<_> = ids.iter().skip(1).cloned().collect();
    let (out, parents) =
        narwhal_types::test_utils::make_optimal_certificates(&committee, 1..=2, &genesis, &nodes);
    certificates.extend(out);

    // Add back the leader for rounds 3 and 4.
    let (out, parents) =
        narwhal_types::test_utils::make_optimal_certificates(&committee, 3..=4, &parents, &ids);
    certificates.extend(out);
...
async fn committed_round_after_restart() {
    ...
    let genesis =
        Certificate::genesis(&committee).iter().map(|x| x.digest()).collect::<BTreeSet<_>>();
    let (certificates, _) = narwhal_types::test_utils::make_certificates_with_epoch(
        &committee,
        1..=11,
        epoch,
        &genesis,
    ...
    for input_round in (1..=11usize).step_by(2) {
        ...
        let (tx_new_certificates, rx_new_certificates) = narwhal_types::test_channel!(100);
        let (tx_primary, mut rx_primary) = narwhal_types::test_channel!(100);
        let (tx_output, mut rx_output) = narwhal_types::test_channel!(100);
...
async fn restart_with_new_committee() {
    ...
    for epoch in 0..5 {
        ...
        let genesis =
            Certificate::genesis(&committee).iter().map(|x| x.digest()).collect::<BTreeSet<_>>();
        let (mut certificates, next_parents) =
            narwhal_types::test_utils::make_certificates_with_epoch(
                &committee,
                1..=2,
                epoch,
                &genesis,
        ...
        // Make two certificate (f+1) with round 3 to trigger the commits.
        let (_, certificate) = narwhal_types::test_utils::mock_certificate_with_epoch(
            &committee,
            ids[0],
            3,
            epoch,
...
async fn garbage_collection_basic() {
    ...
    let (certificates, _round_5_certificates) =
        narwhal_types::test_utils::make_certificates_with_slow_nodes(
            &committee,
            1..=7,
            genesis,
            &ids,
...
async fn slow_node() {
    ...
    let (certificates, _) = narwhal_types::test_utils::make_certificates_with_slow_nodes(
        &committee,
        9..=9,
...
async fn not_enough_support_and_missing_leaders_and_gc() {
    ...
    for id in &keys_with_dead_node {
        ...
        if id == first_node {
            ...
            let (_, certificate) =
                narwhal_types::test_utils::mock_certificate(&committee, *id, 3, parents);
    ...
    for id in ids.iter().filter(|a| *a != missing_leader) {
        ...
        let (_, certificate) =
            narwhal_types::test_utils::mock_certificate(&committee, *id, 4, parents);
