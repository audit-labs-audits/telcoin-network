...
impl PayloadBuilderHandle {
    ...
    // /// the best payload that has been built so far.
    // pub async fn resolve_next_batch(
    //     &self,
    //     id: PayloadId,
    // // ) -> Option<Result<Arc<BuiltPayload>, PayloadBuilderError>> {
    // ) -> oneshot::Receiver<Result<Arc<BuiltPayload>, PayloadBuilderError>> {
    //     let (tx, rx) = oneshot::channel();
    //     self.to_service.send(PayloadServiceCommand::ResolveNextBatch(id, tx)).ok();
    //     rx
    // }
