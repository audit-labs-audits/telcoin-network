...
use alloy::{network::EthereumWallet, providers::{Provider, ProviderBuilder}, sol, sol_types::SolValue};
use clap::Parser;
use gcloud_sdk::{
    google::cloud::kms::v1::{
...
use reth::{
    providers::ExecutionOutcome, tasks::{TaskExecutor, TaskManager}, CliContext
};
...
use tn_types::{
    ...
    execution_outcome_from_test_batch_, TransactionFactory}, TransactionSigned, WorkerBlock,
};
...
//         client.request("eth_getBalance", rpc_params!(default_deployer_address)).await.expect("eth_getBalance issue");
...
async fn test_faucet_transfers_tel_and_xyz_with_google_kms_e2e() -> eyre::Result<()> {
    ...
    // extend genesis accounts to fund factory_address, etch bytecodes, and construct proxy creation txs
    let faucet_impl_address = Address::random();
    ...
    let stablecoin_constructor_params = (stablecoin_impl_address, stablecoin_init_call.clone()).abi_encode_params();
    let stablecoin_create_data = [ERC1967PROXY_INITCODE.as_slice(), &stablecoin_constructor_params[..]].concat();
    ...
    let raw_txs = vec![faucet_tx_raw, stablecoin_tx_raw, role_tx_raw, updatexyz_tx_raw, minter_tx_raw];
    ...
    let genesis_accounts = vec![
        (factory_address, GenesisAccount::default().with_balance(U256::MAX)),
        (kms_address, GenesisAccount::default().with_balance(U256::MAX)),
        (stablecoin_impl_address, GenesisAccount::default().with_code(Some(stablecoin_impl_bytecode.into()))),
        (stablecoin_address, GenesisAccount::default()
    ...
    let tel_tx_hash: String = client.request("faucet_transfer", rpc_params![random_address]).await?;
    ...
    let tx_hash: String = client.request("faucet_transfer", rpc_params![new_random_address]).await?;
    info!(target: "faucet-test", ?tx_hash, "new random faucet request success. waiting for balance increase...");
    ...
    let starting_xyz_balance: U256 = U256::from(stablecoin_contract.balanceOf(new_random_address).call().await?._0);
    ...
    let xyz_tx_hash: String = client.request("faucet_transfer", rpc_params![new_random_address, stablecoin_address]).await?;
    ...
    let result = timeout(duration, async {
        loop {
            let actual_xyz_balance: U256 = stablecoin_contract.balanceOf(new_random_address).call().await?._0;
    ...
    info!(target: "faucet-test", "account balance updated. submitting duplicate request and shutting down...");
    assert!(client
        .request::<String, _>("faucet_transfer", rpc_params![new_random_address, stablecoin_address])
        .await
        .is_err());
