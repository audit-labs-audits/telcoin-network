...
async fn commit_one() {
    ...
    let (tx_primary, mut rx_primary) = lattice_test_utils::test_channel!(1);
    let (tx_output, mut rx_output) = lattice_test_utils::test_channel!(1);
...
async fn committed_round_after_restart() {
    ...
    for input_round in (1..=11usize).step_by(2) {
        ...
        let (tx_new_certificates, rx_new_certificates) = lattice_test_utils::test_channel!(100);
        let (tx_primary, mut rx_primary) = lattice_test_utils::test_channel!(100);
        let (tx_output, mut rx_output) = lattice_test_utils::test_channel!(100);
...
async fn restart_with_new_committee() {
    ...
    for epoch in 0..5 {
        ...
        // Make two certificate (f+1) with round 3 to trigger the commits.
        let (_, certificate) = lattice_test_utils::mock_certificate_with_epoch(
            &committee,
            ids[0],
            3,
            epoch,
