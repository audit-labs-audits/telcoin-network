    ...
    DB: StateProvider,
{
    ...
    fn apply_dao_fork_changes(
        ...
        for address in DAO_HARDKFORK_ACCOUNTS {
            ...
            let old = to_execution_acc(&db_account.info);
            // drain balance
            drained_balance += core::mem::take(&mut db_account.info.balance);
            let new = to_execution_acc(&db_account.info);
        ...
        for (transaction, sender) in block.body.iter().zip(senders) {
            ...
            post_state.add_receipt(
                ...
                Receipt {
                    ...
                    cumulative_gas_used,
                    // convert to reth log
                    logs: result.into_logs().into_iter().map(into_execution_log).collect(),
                },
            );
        }
        ...
        if account.is_destroyed {
            ...
            let cached_account = match db.accounts.entry(address) {
                Entry::Vacant(entry) => {
                    ...
                    let new_account = to_execution_acc(&entry.info);
                ...
                Entry::Occupied(entry) => {
                    ...
                    let old_account = to_execution_acc(&entry.info);
                    let new_account = to_execution_acc(&account.info);
...
mod tests {
    ...
    impl AccountReader for StateProviderTest {
        fn basic_account(&self, address: Address) -> execution_interfaces::Result<Option<Account>> {
    ...
    impl BlockHashReader for StateProviderTest {
        fn block_hash(&self, number: u64) -> execution_interfaces::Result<Option<H256>> {
        ...
        ) -> execution_interfaces::Result<Vec<H256>> {
            let range = start..end;
            Ok(self
                .block_hash
                .iter()
    ...
    impl StateRootProvider for StateProviderTest {
        fn state_root(&self, _post_state: PostState) -> execution_interfaces::Result<H256> {
            todo!()
        }
    }
    ...
    impl StateProvider for StateProviderTest {
        ...
        fn proof(
            ...
            _address: Address,
            _keys: &[H256],
        ) -> execution_interfaces::Result<(Vec<Bytes>, H256, Vec<Vec<Bytes>>)> {
            todo!()
        }
    }
