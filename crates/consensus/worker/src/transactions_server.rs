...
impl<V: TransactionValidator> TxServer<V> {
    ...
    async fn run(mut self) {
        ...
        loop {
            match consensus_network::config::Config::new()
...
impl<V: TransactionValidator> Transactions for TxReceiverHandler<V> {
    ...
    async fn submit_transaction_stream(
        &self,
        request: Request<tonic::Streaming<tn_types::consensus::TransactionProto>>,
    ) -> Result<Response<tn_types::consensus::Empty>, Status> {
