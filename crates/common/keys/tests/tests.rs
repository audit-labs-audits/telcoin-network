...
fn mnemonic_test() {
    ...
    let keystore_path = temp_dir.path().join("lattice.keystore");
    let mut keystore = Keystore::from(FileBasedKeystore::new(&keystore_path).unwrap());
    let (address, phrase, scheme) = keystore
        .generate_and_add_new_key(SignatureScheme::ED25519, None, None)
        .unwrap();

    let keystore_path_2 = temp_dir.path().join("lattice2.keystore");
...
fn keystore_display_test() -> Result<(), anyhow::Error> {
    ...
    assert!(keystore.to_string().contains("lattice.keystore"));
