...
pub(crate) fn impl_encodable(ast: &syn::DeriveInput) -> Result<TokenStream> {
    ...
    let impl_block = quote! {
        trait E {
            fn rlp_header(&self) -> execution_rlp::Header;
        }

        impl #impl_generics E for #name #ty_generics #where_clause {
            fn rlp_header(&self) -> execution_rlp::Header {
                let mut rlp_head = execution_rlp::Header { list: true, payload_length: 0 };
                #(#length_stmts)*
                rlp_head
            }
        }

        impl #impl_generics execution_rlp::Encodable for #name #ty_generics #where_clause {
            fn length(&self) -> usize {
                let rlp_head = E::rlp_header(self);
                return execution_rlp::length_of_length(rlp_head.payload_length) + rlp_head.payload_length;
            }
            fn encode(&self, out: &mut dyn execution_rlp::BufMut) {
...
pub(crate) fn impl_max_encoded_len(ast: &syn::DeriveInput) -> Result<TokenStream> {
    ...
    let impl_block = quote! {
        unsafe impl execution_rlp::MaxEncodedLen<{ execution_rlp::const_add(execution_rlp::length_of_length(#(#stmts)*), #(#stmts)*) }> for #name {}
        unsafe impl execution_rlp::MaxEncodedLenAssoc for #name {
            const LEN: usize = { execution_rlp::const_add(execution_rlp::length_of_length(#(#stmts)*), { #(#stmts)* }) };
        }
    };
...
fn encodable_length<'a>(
    ...
    if is_opt {
        ...
        quote! { rlp_head.payload_length += &self.#ident.as_ref().map(|val| execution_rlp::Encodable::length(val)).unwrap_or(#default); }
    } else {
        quote! { rlp_head.payload_length += execution_rlp::Encodable::length(&self.#ident); }
    }
}
...
fn encodable_max_length(index: usize, field: &syn::Field) -> TokenStream {
    ...
    if index == 0 {
        quote! { <#fieldtype as execution_rlp::MaxEncodedLenAssoc>::LEN }
    } else {
        quote! { + <#fieldtype as execution_rlp::MaxEncodedLenAssoc>::LEN }
    }
}
    ...
    if is_opt {
        let if_some_encode = quote! {
            if let Some(val) = self.#ident.as_ref() {
                execution_rlp::Encodable::encode(val, out)
            }
        };
        ...
        quote! { execution_rlp::Encodable::encode(&self.#ident, out); }
    }
}
