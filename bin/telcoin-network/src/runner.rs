...
impl CliRunner {
        ...
        E: Send + Sync + From<std::io::Error> + From<execution_tasks::PanickedTaskError> + 'static,
    {
        ...
        // warn!(target: "lattice::cli", "Received shutdown signal, waiting up to 30 seconds for
...
/// This is public for using within tests.
...
where
    F: Future<Output = Result<(), E>>,
    E: Send + Sync + From<execution_tasks::PanickedTaskError> + 'static,
{
    {
        pin_mut!(fut);
        tokio::select! {
    ...
    E: Send + Sync + 'static,
{
    ...
    tokio::select! {
        _ = ctrl_c => {
            trace!(target: "lattice::cli",  "Received ctrl-c");
        },
        res = fut => res?,
    }
