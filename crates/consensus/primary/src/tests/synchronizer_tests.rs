...
use lattice_consensus::utils::gc_round;
...
use lattice_test_utils::{
    make_optimal_signed_certificates, mock_signed_certificate,
    CommitteeFixture,
};
...
async fn accept_certificates() {
    ...
    let (tx_parents, mut rx_parents) = lattice_test_utils::test_channel!(4);
...
async fn synchronizer_recover_basic() {
    ...
    let (tx_certificate_fetcher, _rx_certificate_fetcher) = lattice_test_utils::test_channel!(1);
    let (tx_new_certificates, _rx_new_certificates) = lattice_test_utils::test_channel!(3);
    let (tx_parents, _rx_parents) = lattice_test_utils::test_channel!(4);
    let (_tx_consensus_round_updates, rx_consensus_round_updates) =
        watch::channel(ConsensusRound::default());
    let (tx_synchronizer_network, rx_synchronizer_network) = oneshot::channel();
...
async fn synchronizer_recover_previous_round() {
    ...
    let (tx_certificate_fetcher, _rx_certificate_fetcher) = lattice_test_utils::test_channel!(1);
    let (tx_new_certificates, _rx_new_certificates) = lattice_test_utils::test_channel!(6);
    let (tx_parents, _rx_parents) = lattice_test_utils::test_channel!(10);
    let (_tx_consensus_round_updates, rx_consensus_round_updates) =
        watch::channel(ConsensusRound::default());
    let (tx_synchronizer_network, rx_synchronizer_network) = oneshot::channel();
    ...
    let (tx_certificate_fetcher, _rx_certificate_fetcher) = lattice_test_utils::test_channel!(1);
    let (tx_new_certificates, _rx_new_certificates) = lattice_test_utils::test_channel!(6);
    let (tx_parents, mut rx_parents) = lattice_test_utils::test_channel!(10);
    let (tx_synchronizer_network, rx_synchronizer_network) = oneshot::channel();
...
async fn deliver_certificate_not_found_parents() {
    ...
    let (_, certificates_store, payload_store) = create_db_stores();
    let (tx_certificate_fetcher, mut rx_certificate_fetcher) = lattice_test_utils::test_channel!(1);
    let (tx_new_certificates, _rx_new_certificates) = lattice_test_utils::test_channel!(100);
    let (tx_parents, _rx_parents) = lattice_test_utils::test_channel!(100);
    let (_tx_consensus_round_updates, rx_consensus_round_updates) =
        watch::channel(ConsensusRound::default());
...
async fn gc_suspended_certificates() {
    ...
    let (_header_store, certificate_store, payload_store) = create_db_stores();
    let (tx_certificate_fetcher, _rx_certificate_fetcher) = lattice_test_utils::test_channel!(100);
    let (tx_new_certificates, mut rx_new_certificates) = lattice_test_utils::test_channel!(100);
    let (tx_parents, _rx_parents) = lattice_test_utils::test_channel!(100);
    let (tx_consensus_round_updates, rx_consensus_round_updates) =
        watch::channel(ConsensusRound::new(1, 0));
    let (_tx_synchronizer_network, rx_synchronizer_network) = oneshot::channel();
