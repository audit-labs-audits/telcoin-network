...
use execution_provider::{CanonChainTracker, CanonStateNotificationSender, Chain, StateProviderFactory};
use execution_revm::{
    database::{State, SubState},
    executor::Executor,
};
use execution_stages::PipelineEvent;
                    ...
                    match executor.execute_transactions(&block, U256::ZERO, Some(senders.clone())) {
                        Ok((post_state, gas_used)) => {
                            ...
                            let _ = canon_state_notification
                                .send(execution_provider::CanonStateNotification::Commit { new: chain });
                        }
                        Err(err) => {
