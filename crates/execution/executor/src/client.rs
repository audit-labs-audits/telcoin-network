...
impl AutoSealClient {
    ...
    async fn fetch_headers(&self, request: HeadersRequest) -> Vec<Header> {
        trace!(target: "execution::executor", ?request, "received headers request");
        ...
        let mut block: BlockHashOrNumber = match start {
            ...
            BlockHashOrNumber::Number(num) => {
                if let Some(hash) = storage.block_hash(num) {
                    ...
                    warn!(target: "execution::executor", num, "no matching block found");
                    return headers;
                }
            }
        };
        ...
        trace!(target: "execution::executor", ?headers, "returning headers");

        headers
    }

    async fn fetch_bodies(&self, hashes: Vec<B256>) -> Vec<BlockBody> {
        trace!(target: "execution::executor", ?hashes, "received bodies request");
        let storage = self.storage.read().await;
        ...
        trace!(target: "execution::executor", ?bodies, "returning bodies");
