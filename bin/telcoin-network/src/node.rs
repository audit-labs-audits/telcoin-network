...
//         let cmd =
//             NodeCommand::<()>::try_parse_from(["reth", "--discovery.addr",
// "127.0.0.1"]).unwrap();         assert_eq!(cmd.network.discovery.addr, Ipv4Addr::LOCALHOST);
//     }
...
//         let cmd = NodeCommand::<()>::try_parse_from(["reth", "--discovery.port",
// "300"]).unwrap();         assert_eq!(cmd.network.discovery.port, 300);
//     }

//     #[test]
//     fn parse_port() {
//         let cmd =
//             NodeCommand::<()>::try_parse_from(["reth", "--discovery.port", "300", "--port",
// "99"])                 .unwrap();
