...
impl AutoSealClient {
    ...
    async fn fetch_headers(&self, request: HeadersRequest) -> Vec<Header> {
        trace!(target: "execution::batch_maker", ?request, "received headers request");
        ...
        let mut block: BlockHashOrNumber = match start {
            ...
            BlockHashOrNumber::Number(num) => {
                if let Some(hash) = storage.block_hash(num) {
                    ...
                    warn!(target: "execution::batch_maker", num, "no matching block found");
                    return headers
                }
            }
        };
        ...
        trace!(target: "execution::batch_maker", ?headers, "returning headers");

        headers
    }

    async fn fetch_bodies(&self, hashes: Vec<B256>) -> Vec<BlockBody> {
        trace!(target: "execution::batch_maker", ?hashes, "received bodies request");
        let storage = self.storage.read().await;
        ...
        trace!(target: "execution::batch_maker", ?bodies, "returning bodies");
