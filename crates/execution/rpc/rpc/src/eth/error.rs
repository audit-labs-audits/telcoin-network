...
pub enum EthApiError {
    ...
    /// Thrown when a [AccountOverride](execution_rpc_types::state::AccountOverride) contains
    ...
    #[error(transparent)]
    Internal(execution_interfaces::Error),
...
impl From<execution_interfaces::Error> for EthApiError {
    fn from(error: execution_interfaces::Error) -> Self {
        match error {
            execution_interfaces::Error::Provider(err) => err.into(),
            err => EthApiError::Internal(err),
        }
    }
}

impl From<execution_interfaces::provider::ProviderError> for EthApiError {
    fn from(error: execution_interfaces::provider::ProviderError) -> Self {
        use execution_interfaces::provider::ProviderError;
        match error {
