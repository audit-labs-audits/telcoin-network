...
mod error;
pub use error::{
    LatticeConsensusEngineError, LatticeEngineResult, LatticeForkChoiceUpdateError,
...
use self::error::LatticeNextBatchError;
...
pub struct LatticeConsensusEngine<DB, BT, Client>
where
    DB: Database,
    ...
    Client: HeadersClient + BodiesClient + Clone + Unpin + 'static,
{
    ...
    ///  * stage data was dropped manually through `tn stage drop ...`
    ///
    /// # Returns
    ///
    /// A target block hash if the pipeline is inconsistent, otherwise `None`.
    fn check_pipeline_consistency(&self) -> Result<Option<H256>, execution_interfaces::Error> {
    ...
    // TODO: this is not relevant with Lattice Consensus
    //
    /// Returns true if the distance from the local tip to the block is greater than the configured
    /// threshold
    #[inline]
    fn _exceeds_pipeline_run_threshold(&self, local_tip: u64, block: u64) -> bool {
    ...
    fn _can_pipeline_sync_to_finalized(
        &self,
        ...
        let mut exceeds_pipeline_run_threshold =
            self._exceeds_pipeline_run_threshold(canonical_tip_num, target_block_number);
            ...
            .and_then(|state| self.blockchain.buffered_header_by_hash(state.finalized_block_hash))
        {
            ...
            exceeds_pipeline_run_threshold =
                self._exceeds_pipeline_run_threshold(canonical_tip_num, buffered_finalized.number);
        }
    ...
    fn prepare_invalid_response(&self, mut parent_hash: H256) -> BatchPayloadStatus {
        ...
        BatchPayloadStatus::Invalid {
            validation_error: BatchPayloadValidationError::LinksToRejectedPayload.to_string(),
        }
    }
    ...
    fn check_invalid_ancestor(&mut self, head: H256) -> Option<BatchPayloadStatus> {
        let parent_hash = {
    ...
    // // TODO: "forkchoice" is misleading with Lattice consensus
    // //
    // // this method is used to confirm genesis is finalized
    // // it is also used to confirm a cert is finalized
    // //
    ...
    //     self.listeners.notify(LatticeConsensusEngineEvent::ForkchoiceUpdated(state, status));
    ...
    //     state: ForkchoiceState,
    //     attrs: Option<PayloadAttributes>,
    // ) -> Result<OnForkChoiceUpdated, execution_interfaces::Error> {
    //     todo!();
    ...
    //             } else {
    //                 debug!(target: "consensus::engine", fcu_head_num=?outcome.header().number, current_head_num=?self.blockchain.canonical_tip().number, "Ignoring lattice update to old head");
    //             }
    ...
    //             BatchPayloadStatus::new(BatchPayloadStatusEnum::Valid, Some(state.head_block_hash))
    //         }
    //         Err(error) => {
    ...
    //             }),
    //         ) => {
    //             return BatchPayloadStatus::from_status(BatchPayloadStatusEnum::Invalid {
    ...
    //     BatchPayloadStatus::from_status(BatchPayloadStatusEnum::Syncing)
    // }
    ...
    fn try_insert_new_payload(
        ...
        //         self.listeners.notify(LatticeConsensusEngineEvent::CanonicalBlockAdded(block));
        //         BatchPayloadStatusEnum::Valid
        //     }
        //     InsertPayloadOk::Inserted(BlockStatus::Accepted) => {
        //         self.listeners.notify(LatticeConsensusEngineEvent::ForkBlockAdded(block));
        //         // BatchPayloadStatusEnum::Accepted
        //         todo!()
        //     }
        ...
        //         BatchPayloadStatusEnum::Syncing
        //     }
        //     InsertPayloadOk::AlreadySeen(BlockStatus::Valid) => {
        //         latest_valid_hash = Some(block_hash);
        //         BatchPayloadStatusEnum::Valid
        //     }
        //     InsertPayloadOk::AlreadySeen(BlockStatus::Accepted) => BatchPayloadStatusEnum::Accepted,
        // };
        todo!()
        // Ok(BatchPayloadStatus::new(status, latest_valid_hash))
    }
    ...
    //         let status = BatchPayloadStatusEnum::Invalid { validation_error: error.to_string() };
    //         Ok(BatchPayloadStatus::new(status, latest_valid_hash))
    //     } else {
    //         Err(LatticeOnNewPayloadError::Internal(Box::new(error)))
    //     }
    // }
    ...
    //     &mut self,
    //     block_hash: H256,
    // ) -> Result<bool, execution_interfaces::Error> {
    //     todo!();
    ...
    // //
    // // which means we have a batch with a missing parent
    // //
    // // can this happen after consensus (while building the cert block) ?
    // // if so, then we need to handle this case
    // //
    ...
    //     if let Some(target) = self._can_pipeline_sync_to_finalized(
    //         canonical_tip_num,
    ...
    //                 if matches!(
    //                     err,
    //                     execution_interfaces::Error::Execution(
    ...
    //     &mut self,
    //     ev: EngineSyncEvent,
    // ) -> Option<Result<(), LatticeConsensusEngineError>> {
    //     todo!();
    //     match ev {
    //         EngineSyncEvent::FetchedFullBlock(block) => {
    //             // TODO: this is useful for RPC nodes, but not for validators
    ...
    //             return Some(Err(LatticeConsensusEngineError::PipelineChannelClosed))
    //         }
...
impl<DB, BT, Client> Future for LatticeConsensusEngine<DB, BT, Client>
where
    DB: Database + Unpin + 'static,
    Client: HeadersClient + BodiesClient + Clone + Unpin + 'static,
        ...
        loop {
            ...
            match this.engine_message_rx.poll_next_unpin(cx) {
                Poll::Ready(Some(msg)) => match msg {
                    // genesis
                    LatticeEngineMessage::ForkchoiceUpdated { state, payload_attrs, tx } => {
                        todo!()
...
// mod tests {
//     use super::*;
//     use crate::{LatticeForkChoiceUpdateError, LatticeOnNewPayloadError};
...
//     };
//     use execution_db::{test_utils::create_test_rw_db, DatabaseEnv};
//     use execution_interfaces::{
...
//             self.engine_handle.validate_batch(payload.into()).await
//         }
...
//             let (mut engine, handle) = LatticeConsensusEngine::new(
...
//         assert_matches!(
//             res.await,
//             Ok(Err(LatticeConsensusEngineError::Pipeline(n))) if matches!(*n.as_ref(), PipelineError::Stage(StageError::ChannelClosed))
//         );
//     }
...
//         use super::*;
//         use execution_db::{tables, transaction::DbTxMut};
...
//             assert_matches!(
//                 res,
//                 Err(LatticeForkChoiceUpdateError::ForkchoiceUpdateError(
...
//             let expected_result = ForkchoiceUpdated::from_status(BachPayloadStatusEnum::Syncing);
...
//             let expected_result = ForkchoiceUpdated::from_status(BatchPayloadStatusEnum::Valid)
...
//                 })
//                 .await;
//             let expected_result = ForkchoiceUpdated::from_status(BatchPayloadStatusEnum::Syncing);
...
//                 assert_matches!(payload_status.status, BatchPayloadStatusEnum::Invalid { .. });
...
//                 })
//                 .await;
//             let expected_result = ForkchoiceUpdated::from_status(BatchPayloadStatusEnum::Invalid {
...
//             assert_matches!(res, Ok(result) => assert_matches!(result.status, BatchPayloadStatusEnum::Invalid { .. }));

//             // Send new payload
//             let res =
//                 env.send_new_payload(random_block(&mut rng, 1, None, None, Some(0)).into()).await;
//             let expected_result = BatchPayloadStatus::from_status(BatchPayloadStatusEnum::Syncing);
//             assert_matches!(res, Ok(result) => assert_eq!(result, expected_result));
...
//                 })
//                 .await;
//             let expected_result = BatchPayloadStatus::from_status(BatchPayloadStatusEnum::Valid)
...
//             let expected_result = BatchPayloadStatus::from_status(BatchPayloadStatusEnum::Invalid {
//                 validation_error: BlockValidationError::BlockPreMerge { hash: block1.hash }
//                     .to_string(),
//             })
//             .with_latest_valid_hash(H256::zero());
