...
/// Returns the path to the lattice data directory.
///
/// Refer to [dirs_next::data_dir] for cross-platform behavior.
pub fn data_dir() -> Option<PathBuf> {
    dirs_next::data_dir().map(|root| root.join("lattice"))
}

/// Returns the path to the lattice database.
///
/// Refer to [dirs_next::data_dir] for cross-platform behavior.
...
/// Returns the path to the lattice configuration directory.
///
/// Refer to [dirs_next::config_dir] for cross-platform behavior.
pub fn config_dir() -> Option<PathBuf> {
    dirs_next::config_dir().map(|root| root.join("lattice"))
}

/// Returns the path to the lattice cache directory.
///
/// Refer to [dirs_next::cache_dir] for cross-platform behavior.
pub fn cache_dir() -> Option<PathBuf> {
    dirs_next::cache_dir().map(|root| root.join("lattice"))
}

/// Returns the path to the lattice logs directory.
///
/// Refer to [dirs_next::cache_dir] for cross-platform behavior.
...
/// Returns the path to the lattice data dir.
///
...
///
/// ```
/// use telcoin_network::dirs::{PlatformPath, DataDirPath};
/// use std::str::FromStr;
///
...
impl<D: XdgPath> MaybePlatformPath<D> {
    ...
    pub fn temp_chain(&self, chain: Chain) -> ChainPath<D> {
...
/// Wrapper type around PlatformPath that includes a `Chain`, used for separating lattice data for
/// different networks.
///
...
impl<D> ChainPath<D> {
    ...
    /// Returns the path to the lattice p2p secret key for this chain.
...
mod tests {
    ...
    fn test_maybe_data_dir_path() {
        ...
        assert!(path.as_ref().ends_with("lattice/mainnet"), "{:?}", path);

        let db_path = path.db_path();
        assert!(db_path.ends_with("lattice/mainnet/db"), "{:?}", db_path);
