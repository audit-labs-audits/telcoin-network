...
impl Decodable for EnrResponse {
    fn decode(buf: &mut &[u8]) -> Result<Self, DecodeError> {
        ...
        let this = Self {
            request_hash: execution_rlp::Decodable::decode(b)?,
            enr: EnrWrapper::<SecretKey>::decode(b)?,
        };
        // TODO: `Decodable` can be derived once we have native execution_rlp decoding for ENR: <https://github.com/paradigmxyz/reth/issues/482>
