    ...
    Client: HeadersClient + BodiesClient + Clone + Unpin + 'static,
{
    ...
    pub fn with_channel(
        ...
        let handle = LatticeConsensusEngineHandle {
            to_engine,
            payload_store: payload_builder.clone().into(),
        };
    ...
    /// Ensures that the given batch does not violate any consensus rules that concern the block's
    /// layout, like:
    ///    - batch version
        ...
        loop {
            ...
            match this.engine_message_rx.poll_next_unpin(cx) {
                Poll::Ready(Some(msg)) => match msg {
                    ...
                    LatticeEngineMessage::NewBatch { batch, tx } => {
                        this.metrics.new_payload_messages.increment(1);
                        let res = this.on_new_payload(batch.into());
                        let _ = tx.send(res);
                    }
...
mod tests {
    ...
    impl<DB> TestEnv<DB> {
        ...
        async fn send_new_payload(
            ...
            self.engine_handle.new_batch_from_peer(payload.into()).await
        }
