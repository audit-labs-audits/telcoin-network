...
use execution_rlp::{length_of_length, Decodable, Encodable, EMPTY_STRING_CODE};
...
impl Encodable for Header {
    fn encode(&self, out: &mut dyn BufMut) {
        let list_header =
            execution_rlp::Header { list: true, payload_length: self.header_payload_length() };
...
impl Decodable for Header {
    fn decode(buf: &mut &[u8]) -> Result<Self, execution_rlp::DecodeError> {
        let rlp_head = execution_rlp::Header::decode(buf)?;
        if !rlp_head.list {
