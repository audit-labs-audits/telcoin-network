...
mod tests {
    ...
    use crate::{
        adiri_chain_spec, adiri_genesis, generate_proof_of_possession, test_utils::contract_artifacts::{ERC1967PROXY_INITCODE, ERC1967PROXY_RUNTIMECODE}, BlsKeypair, Multiaddr, NetworkKeypair, PrimaryInfo, TelcoinDirs, ValidatorInfo, WorkerBlock, WorkerIndex, WorkerInfo
    };
    ...
    use tn_types::{
        WorkerBlock as OtherWorkerBlock,
        test_utils::{
    ...
    #[test]
    fn test_genesis_with_consensus_registry() {
        ...
        let execution_node = default_test_execution_node(Some(pre_genesis_chain.clone()));
        let provider = execution_node.get_provider().await;
        let block_executor = execution_node.get_block_executor().await;
        // execute batch
        let batch = OtherWorkerBlock::new(raw_txs, SealedHeader::default());
