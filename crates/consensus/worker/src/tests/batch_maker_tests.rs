...
use tn_network_types::{MockWorkerToPrimary, MockWorkerToEngine};
use tokio::sync::mpsc;
...
async fn request_next_batch() {
    ...
    // client.set_worker_to_primary_local_handler(Arc::new(mock_worker_to_primary));
    // mock engine
    let id = 0;
    ...
    client.set_worker_to_engine_local_handler(Arc::new(mock_worker_to_engine));
    ...
    // let the batch maker spawn
    tokio::task::yield_now().await;

    // sleep long enough to ensure the timer expired
