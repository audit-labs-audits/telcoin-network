...
fn malloc_size_of_derive(s: synstructure::Structure) -> proc_macro2::TokenStream {
        ...
        if ignore {
            ...
            Some(quote! {
                for item in #binding.iter() {
                    sum += mem_utils::MallocSizeOf::size_of(item, ops);
                }
            })
        } else {
            Some(quote! {
                sum += mem_utils::MallocSizeOf::size_of(#binding, ops);
            })
        }
    });
    ...
    for param in ast.generics.type_params() {
        ...
        where_clause.predicates.push(parse_quote!(#ident: mem_utils::MallocSizeOf));
    }

    let tokens = quote! {
        impl #impl_generics mem_utils::MallocSizeOf for #name #ty_generics #where_clause {
            #[inline]
            #[allow(unused_variables, unused_mut, unreachable_code)]
            fn size_of(&self, ops: &mut mem_utils::MallocSizeOfOps) -> usize {
                let mut sum = 0;
                match *self {
                    #match_body
                }
