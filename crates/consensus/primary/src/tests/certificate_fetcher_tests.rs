...
//     // Send a primary message for a certificate with parents that do not exist locally, to
// trigger fetching.     let target_index = 123;
//     assert!(!synchronizer
...
// certificate 123.     loop {
//         match rx_fetch_req.recv().await {
//             Some(r) => {
//                 let (_, skip_rounds) = r.get_bounds();
//                 if skip_rounds.values().next().unwrap().len() == 1 {
//                     // Drain the fetch requests sent out before the last reply, when only 1 round
// in skip_rounds.                     tx_fetch_resp.try_send(first_batch_resp.clone()).unwrap();
//                     continue;
//                 }
//                 req = r;
//                 break;
