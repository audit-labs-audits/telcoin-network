...
/// [`Certificate`] and [`Round`]-aware  variant of the Dag, with a secondary index to link a
/// (pubkey, round) pair to the possible certified collection by that authority at that round.
struct InnerDag {
...
impl InnerDag {
    ...
    fn new(
        ...
        let mut idg = InnerDag { rx_primary, rx_commands, dag, vertices, metrics, rx_shutdown };
    ...
    /// Returns whether the node is still in the Dag as a strong reference, i.e. that it hasn't been
    /// removed through compression. For the purposes of this memory-conscious graph, this is
    /// just "contains" semantics.
    fn contains(&self, digest: CertificateDigest) -> bool {
        self.dag.contains_live(digest)
    }

    /// Returns whether the dag has ever contained a node with the provided digest. The method will
    /// return true either when the node is live (uncompressed) or has been already compressed
    /// as still exists as weak reference.
    ...
    fn rounds(
        ...
        // Our garbage collection is a mark-and-sweep algorithm, where the mark part is in
        // `make_compressible` and `read_causal` triggers a sweep.
        // To make sure we don't return rounds as live when wouldn't be seen as such from a
        // subsequent `read_causal` we need to trigger the sweep in the first place.
        ...
        // TODO: this may become a big source of latency if the sweep has a lot of work to do! Make
        // sure read_causal calls are triggered from heads of the DAG in a background
        // thread, and scheduled after calling remove.
        // - Subsequent `read_causal` calls will be cheaper,
        // - those background read_causals should flip a dirty bit here, so we maintain the
        //   invariant of at most one graph sweep between a remove_collections and a `rounds` call
        ...
        let (earliest, latest) = {
            ...
            // In non-pathological cases, the range is non-empty, and has a lot of dropped nodes
            // towards the beginning yet this can't be a take_while because the DAG
            // removal may be non-contiguous.
            //
    ...
    fn node_read_causal(
        ...
        let vertices = self.vertices.read().unwrap();
        let start_digest = vertices
            .get(&(origin, round))
            .ok_or(ValidatorDagError::NoCertificateForCoordinates(origin, round))?;
    ...
    fn remove(&mut self, digests: Vec<CertificateDigest>) -> Result<(), ValidatorDagError> {
        {
            ...
            let dag_removal_results =
                digests.iter().map(|digest| self.dag.make_compressible(*digest));
...
impl DagHandle {
    ...
    ///
    /// Note: the caller is responsible for validation of the certificate, including, but not
    /// limited to:
    ...
    pub async fn insert(&self, certificate: Certificate) -> Result<(), ValidatorDagError> {
        ...
        if let Err(e) =
            self.tx_commands.send(DagCommand::Insert(Box::new(certificate), sender)).await
        {
            panic!("Failed to send Insert command to store: {e}");
        }
        receiver.await.expect("Failed to receive reply to Insert command from store")
    }

    /// Returns whether the node is still in the Dag as a strong reference, i.e. that it hasn't ben
    /// removed through compression. For the purposes of this memory-conscious graph, this is
    /// just "contains" semantics.
    pub async fn contains(&self, digest: CertificateDigest) -> bool {
        let (sender, receiver) = oneshot::channel();
        if let Err(e) = self.tx_commands.send(DagCommand::Contains(digest, sender)).await {
            panic!("Failed to send Contains command to store: {e}");
        }
        receiver.await.expect("Failed to receive reply to Contains command from store")
    }
    ...
    pub async fn has_ever_contained(&self, digest: CertificateDigest) -> bool {
        let (sender, receiver) = oneshot::channel();
        if let Err(e) = self.tx_commands.send(DagCommand::HasEverContained(digest, sender)).await {
            panic!("Failed to send HasEverContained command to store: {e}");
        }
        receiver.await.expect("Failed to receive reply to HasEverContained command from store")
    }
    ...
    pub async fn rounds(
        ...
        let (sender, receiver) = oneshot::channel();
        if let Err(e) = self.tx_commands.send(DagCommand::Rounds(origin, sender)).await {
            panic!("Failed to send Rounds command to store: {e}");
        }
        receiver.await.expect("Failed to receive reply to Rounds command from store")
    }
    ...
    pub async fn read_causal(
        ...
        let (sender, receiver) = oneshot::channel();
        if let Err(e) = self.tx_commands.send(DagCommand::ReadCausal(start, sender)).await {
            panic!("Failed to send ReadCausal command to store: {e}");
        }
        receiver.await.expect("Failed to receive reply to ReadCausal command from store")
    }
    ...
    pub async fn node_read_causal(
        ...
        let (sender, receiver) = oneshot::channel();
        if let Err(e) =
            self.tx_commands.send(DagCommand::NodeReadCausal((origin, round), sender)).await
        {
            panic!("Failed to send NodeReadCausal command to store: {e}");
        }
        receiver.await.expect("Failed to receive reply to NodeReadCausal command from store")
    }

    /// Removes certificates from the Dag, reclaiming memory in the process.
    ///
    /// Note: If some digests are unknown to the Dag, this will return an error, but will
    /// nonetheless delete the certificates for known digests which are removable.
    pub async fn remove<J: Borrow<CertificateDigest>>(
        ...
        if let Err(e) = self
            .tx_commands
            .send(DagCommand::Remove(digest.into_iter().map(|k| *k.borrow()).collect(), sender))
            .await
        {
            panic!("Failed to send Remove command to store: {e}");
        }
        receiver.await.expect("Failed to receive reply to Remove command from store")
    }
    ...
    pub async fn notify_read(
        ...
        let (sender, receiver) = oneshot::channel();
        if let Err(e) = self.tx_commands.send(DagCommand::NotifyRead(digest, sender)).await {
            panic!("Failed to send NotifyRead command: {e}");
        }
        receiver.await.expect("Failed to receive reply to NotifyRead command")
    }
}
