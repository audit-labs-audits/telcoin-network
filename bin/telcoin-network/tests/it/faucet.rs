...
async fn test_faucet_transfers_tel_and_xyz_with_google_kms_e2e() -> eyre::Result<()> {
    ...
    let faucet_deployed_bytecode =
        RethEnv::fetch_from_json_str(&faucet_standard_json, Some("deployedBytecode.object"))?.as_str().map(|s| hex::decode(s)).unwrap()?;
    let stablecoin_json =
    ...
    let proxy_bytecode = RethEnv::fetch_from_json_str(&proxy_json, Some("deployedBytecode.object"))?.as_str().map(|s| hex::decode(s)).unwrap()?;
