...
//! common logs and metrics destinations, etc.  into a easy to configure common package.  There are
//! also some unique layers such as one to automatically create Prometheus latency histograms for
//! spans.
//!
//! We also purposely separate out logging levels from span creation.  This is often needed by
//! production apps as normally it is not desired to log at very high levels, but still desirable to
//! gather sampled span data all the way down to TRACE level spans.
//!
...
//! ```
//!
//! It is important to retain the guard until the end of the program.  Assign it in the main fn and
//! keep it, for once it drops then log output will stop.
//!
//! There is a builder API available: just do `TelemetryConfig::new()...` Another convenient
//! initialization method is `TelemetryConfig::new().with_env()` to populate the config from
//! environment vars.
//!
...
//!
//! By default, logs (but not spans) are formatted for human readability and output to stdout, with
//! key-value tags at the end of every line. `RUST_LOG` can be configured for custom logging output,
//! including filtering.
//!
...
//!
//! Detailed span start and end logs can be generated by defining the `json_log_output` config
//! variable.  Note that this causes all output to be in JSON format, which is not as
//! human-readable, so it is not enabled by default. This output can easily be fed to backends such
//! as ElasticSearch for indexing, alerts, aggregation, and analysis.
//!
//! NOTE: JSON output requires the `json` crate feature to be enabled.
//!
//! ### Automatic Prometheus span latencies
//!
//! Included in this library is a tracing-subscriber layer named `PrometheusSpanLatencyLayer`.  It
//! will create a Prometheus histogram to track latencies for every span in your app, which is super
//! convenient for tracking span performance in production apps.
//!
//! Enabling this layer can only be done programmatically, by passing in a Prometheus registry to
//! `TelemetryConfig`.
//!
//! ### Span levels vs log levels
//!
//! What spans are included for automatic span latencies, etc.?  These are controlled by
//! the `span_level` config attribute, or the `TS_SPAN_LEVEL` environment variable.  Note that this
//! is separate from `RUST_LOG`, so that you can separately control the logging verbosity from the
//! level of spans that are to be recorded and traced.
//!
...
//! 2. Set the `tokio_console` config setting when running your app (or set TOKIO_CONSOLE env var if
//! using config `with_env()` method) 3. Clone the console repo and `cargo run` to launch the
//! console
//!
//! NOTE: setting tokio TRACE logs is NOT necessary.  It says that in the docs but there's no need
//! to change Tokio logging levels at all.  The console subscriber has a special filter enabled
//! taking care of that.
//!
//! By default, Tokio console listens on port 6669.  To change this setting as well as other setting
//! such as the retention policy, please see the [configuration](https://docs.rs/console-subscriber/latest/console_subscriber/struct.Builder.html#configuration) guide.
//!
//! ### Custom panic hook
//!
//! This library installs a custom panic hook which records a log (event) at ERROR level using the
//! tracing crate.  This allows span information from the panic to be properly recorded as well.
//!
...
use tracing::{metadata::LevelFilter, subscriber::Interest, Level, Metadata, Subscriber};
...
impl FilterHandle {
    ...
    pub fn get(&self) -> Result<String, BoxError> {
        self.0.with_current(|filter| filter.to_string()).map_err(Into::into)
    }
}
...
impl TelemetryConfig {
    ...
    pub fn init(self) -> (TelemetryGuards, FilterHandle) {
        ...
        // NOTE: we don't want to use this to filter all layers.  That causes problems for layers
        // with different filtering needs, including tokio-console/console-subscriber, and
        // it also doesn't fit with the span creation needs for distributed tracing and
        // other span-based tools.
        ...
        // The guard must be returned and kept in the main fn of the app, as when it's dropped then
        // the output gets flushed and closed. If this is dropped too early then no output
        // will appear!
...
impl SamplingFilter {
    fn new(sample_nth: usize, target_prefix: Option<String>) -> Self {
        SamplingFilter { counter: AtomicUsize::new(0), sample_nth, target_prefix }
    }
...
mod tests {
    ...
    fn test_telemetry_init() {
        ...
        // Default logging level is INFO, but here we set the span level to DEBUG.  TRACE spans
        // should be ignored.
        let config =
            TelemetryConfig::new().with_span_level(Level::DEBUG).with_prom_registry(&registry);
