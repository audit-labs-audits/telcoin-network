...
For external types, you'll need to implement the trait here in [`external_impls.rs`](https://github.com/MystenLabs/blob/main/crates/consensus-util-mem/src/external_impls.rs). See the existing implementations in that file for examples.
...
To compute the heap usage of an annotated type at runtime, simply call `consensus_util_mem::malloc_size(&my_struct)`. For complete memory usage, add in the inline size of the type as well, as in:

```rust
consensus_util_mem::malloc_size(&my_struct) + std::mem::size_of::<MyStruct>()
```
