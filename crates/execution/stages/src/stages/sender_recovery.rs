...
/// in [`TxSenders`][execution_db::tables::TxSenders] table.
#[derive(Clone, Debug)]
...
impl<DB: Database> Stage<DB> for SenderRecoveryStage {
    ...
    /// [`BlockBodyIndices`][execution_db::tables::BlockBodyIndices],
    /// collect transactions within that range,
    /// recover signer for each transaction and store entries in
    /// the [`TxSenders`][execution_db::tables::TxSenders] table.
    async fn execute(
        &mut self,
