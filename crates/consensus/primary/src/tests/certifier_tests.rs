...
async fn propose_header() {
    ...
    let (tx_headers, rx_headers) = lattice_test_utils::test_channel!(1);
    let (tx_new_certificates, mut rx_new_certificates) = lattice_test_utils::test_channel!(3);
    let (tx_parents, _rx_parents) = lattice_test_utils::test_channel!(1);
...
async fn shutdown_core() {
    ...
    let mut tx_shutdown = PreSubscribedBroadcastSender::new(NUM_SHUTDOWN_RECEIVERS);
    let (tx_certificate_fetcher, _rx_certificate_fetcher) = lattice_test_utils::test_channel!(1);
    let (_tx_headers, rx_headers) = lattice_test_utils::test_channel!(1);
    let (tx_new_certificates, _rx_new_certificates) = lattice_test_utils::test_channel!(1);
    let (tx_parents, _rx_parents) = lattice_test_utils::test_channel!(1);
    let (_tx_consensus_round_updates, rx_consensus_round_updates) =
        watch::channel(ConsensusRound::new(0, 0));
    let (_tx_synchronizer_network, rx_synchronizer_network) = oneshot::channel();
