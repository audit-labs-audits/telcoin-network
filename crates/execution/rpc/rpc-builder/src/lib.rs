...
//! use execution_provider::{BlockReaderIdExt, ChainSpecProvider, CanonStateSubscriptions, StateProviderFactory, EvmEnvProvider};
//! use execution_rpc_builder::{RethRpcModule, RpcModuleBuilder, RpcServerConfig, ServerBuilder, TransportRpcModuleConfig};
...
//! use execution_rpc_builder::{RethRpcModule, RpcModuleBuilder, RpcServerConfig, TransportRpcModuleConfig};
//! use execution_tasks::TokioTaskExecutor;
//! use execution_transaction_pool::TransactionPool;
//! use execution_rpc_api::EngineApiServer;
//! use execution_rpc_builder::auth::AuthServerConfig;
...
///
/// ```
/// use execution_rpc_builder::{RethRpcModule, RpcModuleSelection};
    ...
    Events: CanonStateSubscriptions + Clone + 'static,
{
    ...
    pub fn module_for(&mut self, config: &RpcModuleSelection) -> RpcModule<()> {
        let mut module = RpcModule::new(());
        let all_methods = self.execution_methods(config.iter_selection());
...
///
/// ```
/// use execution_rpc_builder::{RethRpcModule, TransportRpcModuleConfig};
