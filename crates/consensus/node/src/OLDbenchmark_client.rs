...
use rand::Rng;
use reth_tracing::tracing_subscriber::{self, filter::EnvFilter};
use tokio::{
    net::TcpStream,
...
/// Optionally the --nodes parameter can be passed where a list (comma separated string) of worker
/// addresses should be passed. The benchmarking client will first try to connect to all of those
/// nodes before start sending any transactions. That confirms the system is up and running and
/// ready to start processing the transactions.
#[derive(Parser)]
