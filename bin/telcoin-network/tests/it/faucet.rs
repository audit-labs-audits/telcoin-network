...
#[tokio::test]
async fn test_faucet_transfers_stablecoin_with_google_kms_e2e() -> eyre::Result<()> {
    init_test_tracing();
    ...
    let random_tx_factory = TransactionFactory::new_random();
    let random_address = random_tx_factory.address();
    ...
    let stablecoin_contract = Stablecoin::new(stablecoin_address, provider);
    let starting_balance = stablecoin_contract.balanceOf(random_address).call().await?._0;
    debug!(target: "faucet-test", "starting balance: {starting_balance:?}");
    assert_eq!(starting_balance, U256::ZERO);

    let tx_hash: String = client.request("faucet_transfer", rpc_params![random_address, stablecoin_address]).await?;
    info!(target: "faucet-test", ?tx_hash, "valid faucet transfer tx hash");
    ...
    // // ensure account balance increased
    // let expected_balance = U256::from_str("0xde0b6b3a7640000")?; // 1*10^18 (1 TEL)
    // let actual_balance = stablecoin_contract.balanceOf(random_address).call().await?._0;
