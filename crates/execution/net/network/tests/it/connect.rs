...
async fn test_incoming_node_id_blacklist() {
    ...
    tokio::time::timeout(GETH_TIMEOUT, async move {
        ...
        let (execution_p2p, execution_disc) = unused_tcp_udp();
        let config = NetworkConfigBuilder::new(secret_key)
            .listener_addr(execution_p2p)
            .discovery_addr(execution_disc)
        ...
        let our_enode = NodeRecord::new(execution_p2p, *handle.peer_id());
