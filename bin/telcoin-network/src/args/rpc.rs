...
impl RpcServerArgs {
    ...
    pub(crate) fn jwt_secret(&self, default_jwt_path: PathBuf) -> Result<JwtSecret, JwtError> {
        match self.auth_jwtsecret.as_ref() {
            Some(fpath) => {
                debug!(target: "tn::cli", user_path=?fpath, "Reading JWT auth secret file");
                JwtSecret::from_file(fpath)
            }
            None => {
                if default_jwt_path.exists() {
                    debug!(target: "tn::cli", ?default_jwt_path, "Reading JWT auth secret file");
                    JwtSecret::from_file(&default_jwt_path)
                } else {
                    info!(target: "tn::cli", ?default_jwt_path, "Creating JWT auth secret file");
        ...
        Engine: EngineApiServer,
    {
        ...
        debug!(target: "tn::cli", http=?module_config.http(), ws=?module_config.ws(), "Using RPC module config");
        ...
        let launch_rpc = rpc_modules.start_server(server_config).map_ok(|handle| {
            if let Some(url) = handle.ipc_endpoint() {
                info!(target: "tn::cli", url=%url, "RPC IPC server started");
            }
            if let Some(addr) = handle.http_local_addr() {
                info!(target: "tn::cli", url=%addr, "RPC HTTP server started");
            }
            if let Some(addr) = handle.ws_local_addr() {
                info!(target: "tn::cli", url=%addr, "RPC WS server started");
            }
            handle
        });

        let launch_auth = auth_module.start_server(auth_config).inspect(|_| {
            info!(target: "tn::cli", "RPC auth server started");
        });
        ...
        Tasks: TaskSpawner + Clone + 'static,
    {
        ...
        execution_rpc_builder::auth::launch(
            provider,
            pool,
            network,
            executor,
...
mod tests {
    ...
    fn test_rpc_server_args_parser() {
        let args =
            CommandParser::<RpcServerArgs>::parse_from(["tn", "--http.api", "eth,admin,debug"])
                .args;
