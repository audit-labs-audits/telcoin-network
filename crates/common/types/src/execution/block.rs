...
impl BlockBody {
    ...
    pub fn calculate_tx_root(&self) -> H256 {
        crate::execution::proofs::calculate_transaction_root(&self.transactions)
    }

    /// Calculate the ommers root for the block body.
    pub fn calculate_ommers_root(&self) -> H256 {
        crate::execution::proofs::calculate_ommers_root(&self.ommers)
    }

    /// Calculate the withdrawals root for the block body, if withdrawals exist. If there are no
    /// withdrawals, this will return `None`.
    pub fn calculate_withdrawals_root(&self) -> Option<H256> {
        self.withdrawals.as_ref().map(|w| crate::execution::proofs::calculate_withdrawals_root(w))
    }
