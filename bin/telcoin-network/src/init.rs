...
pub enum InitDatabaseError {
    ...
    #[error(transparent)]
    DBError(#[from] execution_db::DatabaseError),

    /// Internal error.
    #[error(transparent)]
    InternalError(#[from] execution_interfaces::Error),
}
...
pub fn insert_genesis_state<DB: Database>(
    ...
    for (address, account) in &genesis.alloc {
        ...
        if let Some(storage) = &account.storage {
            let mut storage_changes = execution_provider::post_state::StorageChangeset::new();
