//! TODO: good to remove this
//! 
//! The worker's test is in bin/telcoin/network/tests 
//! and tests the same functionality as these, except
//! for getting peers from admin server (last test).
//! 
//! Leaving this commented out for now because there
//! is some helpful code to review for setting up local
//! networks and peer <-> peer servers for worker comms
//! between nodes. This will be helpful in the future
//! when testing validation for peer batches.
...
//     BatchAPI, PreSubscribedBroadcastSender,
// };
// use tn_network_types::{
//     MockWorkerToPrimary, MockWorkerToWorker,
//     WorkerBatchMessage, WorkerToWorkerClient, MockWorkerToEngine,
...
//     // // mock servers
//     // let mut mock_worker_to_engine = MockWorkerToEngine::new();
//     // mock_worker_to_engine.expect_build_batch()
...
//     let (tx_await_batch, mut rx_await_batch) = lattice_test_utils::test_channel!(CHANNEL_CAPACITY);
