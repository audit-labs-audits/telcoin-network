...
use execution_codecs::{main_codec, Compact, CompactZstd};
use execution_rlp::{length_of_length, Decodable, Encodable};
...
impl ReceiptWithBloom {
    ...
    fn decode_receipt(buf: &mut &[u8], tx_type: TxType) -> Result<Self, execution_rlp::DecodeError> {
        ...
        if !rlp_head.list {
            return Err(execution_rlp::DecodeError::UnexpectedString)
        }
        let started_len = b.len();

        let success = execution_rlp::Decodable::decode(b)?;
        let cumulative_gas_used = execution_rlp::Decodable::decode(b)?;
        let bloom = Decodable::decode(b)?;
        let logs = execution_rlp::Decodable::decode(b)?;

        let this = Self { receipt: Receipt { tx_type, success, cumulative_gas_used, logs }, bloom };
        let consumed = started_len - b.len();
        if consumed != rlp_head.payload_length {
            return Err(execution_rlp::DecodeError::ListLengthMismatch {
...
impl Decodable for ReceiptWithBloom {
    fn decode(buf: &mut &[u8]) -> Result<Self, execution_rlp::DecodeError> {
        ...
        match rlp_type.cmp(&execution_rlp::EMPTY_LIST_CODE) {
            Ordering::Less => {
                ...
                let _header = execution_rlp::Header::decode(buf)?;
                let receipt_type = *buf.first().ok_or(execution_rlp::DecodeError::Custom(
...
impl<'a> ReceiptWithBloomEncoder<'a> {
    ...
    fn receipt_rlp_header(&self) -> execution_rlp::Header {
    ...
    fn encode_inner(&self, out: &mut dyn BufMut, with_header: bool) {
        ...
        if with_header {
            ...
            let header = execution_rlp::Header { list: false, payload_length };
            header.encode(out);
        }
        ...
        let receipt = Receipt {
            ...
            logs: vec![
                Log {
                    ...
                    .unwrap()],
                    data: crate::execution::Bytes::from(vec![1; 0xffffff]),
                },
                Log {
